...
SlopeDist <- readRDS(file ="./Rscripts/dHRUM_setup/outputs/SlopeDist_BP_D_FG.rds")
# write.csv(SlopeDist,"./setups_with_opt/BP_D_FG/Slope.csv", row.names = F)


indexF <- which(SoilBP$Land_Use=="Forest")
indexA <- which(SoilBP$Land_Use=="Arable land" | SoilBP$Land_Use=="Meadow")
indexW <- which(SoilBP$Land_Use=="Wetland")

# Soil Storage
# C_Max, CMIN and B_Soil
CCB <- readRDS(file ="./Rscripts/dHRUM_setup/outputs/CCB_BP_D_FG.rds")
...
# ParDFlow1$C_MAX <- CCB$Cmax - (C_diff*0.1)
...
# ParDFup1$B_SOIL <- CCB$b + (abs(CCB$b) * 0.2)
# ParDFlow1$B_SOIL <- CCB$b - (abs(CCB$b)*0.2)
...
# ParDFlow1$CMIN[indexW] <- CCB$Cmin[indexW] - (CCB$Cmin[indexW]*0.9)

# ADIV constraints
# ParDFup1$ADIV[indexF] <- SlopeDist$Slope[indexF] - (SlopeDist$Slope[indexF] * 0.05)  #ADIV up Forest
# ParDFlow1$ADIV[indexF] <- SlopeDist$Slope[indexF] - (SlopeDist$Slope[indexF] * 0.9)  #ADIV low Forest
...
# ParDFup1$ADIV[indexA] <- SlopeDist$Slope[indexA] + (SlopeDist$Slope[indexA] * 0.4) #ADIV up Arable land
# ParDFlow1$ADIV[indexA] <- SlopeDist$Slope[indexA] - (SlopeDist$Slope[indexA] * 0.05) #ADIV up Arable land

# ParDFup1$ADIV[indexW] <- SlopeDist$Slope[indexW] - (SlopeDist$Slope[indexW] * 0.1)  #ADIV up Forest
# ParDFlow1$ADIV[indexW] <- SlopeDist$Slope[indexW] - (SlopeDist$Slope[indexW] * 0.9)  #ADIV up Forest
...
# # ParDFup1$KS[indexF] <- SlopeDist$meanDS[indexF] - (SlopeDist$meanDS[indexF] * 0.05) #KS up Forest
# # ParDFlow1$KS[indexF] <- SlopeDist$meanDS[indexF]- (SlopeDist$meanDS[indexF] * 0.8) #KS low Forest
...
# 
# # ParDFup1$KS[indexA] <- SlopeDist$meanDS[indexA] + (SlopeDist$meanDS[indexA] * 0.5) #KS up Arable land
# # ParDFlow1$KS[indexA] <- SlopeDist$meanDS[indexA] - (SlopeDist$meanDS[indexA] * 0.5)  #KS low Arable land
...
# 
# # ParDFup1$KS[indexW] <- SlopeDist$meanDS[indexW] - (SlopeDist$meanDS[indexW] * 0.05) #KS up Wetland
# # ParDFlow1$KS[indexW] <- SlopeDist$meanDS[indexW] - (SlopeDist$meanDS[indexW] * 0.9)  #KS low Wetland
...
# KF
# ParDFup1$KF[indexF] <-  (SlopeDist$meanDS[indexF] * 0.4) #KS up Forest
# ParDFlow1$KF[indexF] <- 0 #SlopeDist$meanDS[indexF] - (SlopeDist$meanDS[indexF] * 0.4)                                      #KS low Forest
#
# ParDFup1$KF[indexA] <- SlopeDist$meanDS[indexA] + (SlopeDist$meanDS[indexA] * 0.4)  #KS up Arable land
# ParDFlow1$KF[indexA] <- SlopeDist$meanDS[indexA] - (SlopeDist$meanDS[indexA] * 0.2)  #KS low Arable land
#
# ParDFup1$KF[indexW] <- SlopeDist$meanDS[indexW] # SlopeDist$meanDS[indexW] + (SlopeDist$meanDS[indexW] * 0.4)  #KF up Wetland
# ParDFlow1$KF[indexW] <- SlopeDist$meanDS[indexW] + (SlopeDist$meanDS[indexW] * 0.2)  #KF low Wetland
...
# Constraining RETCAP
SDC = readRDS(file = "./Rscripts/dHRUM_setup/outputs/SDC_BP_D_FG.rds")
# ParDFup1$RETCAP[indexF] <- SDC$SDC_2[indexF]*10 - ((SDC$SDC_2[indexF]*10) * 0)
# ParDFlow1$RETCAP[indexF] <- SDC$SDC_2[indexF]*10 - ((SDC$SDC_2[indexF]*10) * 0.50)
...
ParDFup1$RETCAP[indexA] <- SDC$SDC_2[indexA]*10 + ((SDC$SDC_2[indexA]*10) * 0.8)
ParDFlow1$RETCAP[indexA] <- SDC$SDC_2[indexA]*10 - ((SDC$SDC_2[indexA]*10) * 0.30)
# ParDFup1$RETCAP[indexA] <- SDC$SDC_2[indexA]*10 + ((SDC$SDC_2[indexA]*10) * 0.20)
# ParDFlow1$RETCAP[indexA] <- SDC$SDC_2[indexA]*10 - ((SDC$SDC_2[indexA]*10) * 0.20)

# ParDFup1$RETCAP[indexW] <- SDC$SDC_2[indexW]*10 - ((SDC$SDC_2[indexW]*10) * 0.30)
# ParDFlow1$RETCAP[indexW] <- SDC$SDC_2[indexW]*10 - ((SDC$SDC_2[indexW]*10) * 0.50)
...
# CAN_ST
# ParDFup1$CAN_ST[indexF] <- 8       # Forest Up         STEM_ST = 1.5
# ParDFlow1$CAN_ST[indexF] <- 0.5      # Forest Low
